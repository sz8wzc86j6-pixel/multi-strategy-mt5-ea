//+------------------------------------------------------------------+
//|  v6 EA — AVAXUSDT — Optimized for PrimeXBT MT5                    |
//|  Timeframe: 1h | Config: session_wt | Filter: minimal              |
//|  Backtest (14d): 11 trades | 91% WR | PF 6.3             |
//|  Profit: $+1,673 (+516%) | DD: 22%                      |
//+------------------------------------------------------------------+
#property copyright "v6 EA — AVAXUSDT"
#property version   "6.00"
#property strict
#property description "AVAXUSDT 1h | session_wt | 11T 91%WR PF:6.3 | $+1,673 (+516%)"

#include <Trade\Trade.mqh>
#include <Trade\SymbolInfo.mqh>

//+------------------------------------------------------------------+
//| Inputs — Pre-optimized for AVAXUSDT                                |
//+------------------------------------------------------------------+
input group "════ RISK ════"
...
input int     Inp_BaseMagic    = 657538;
...
input bool    Inp_UseSessionWt  = true;  // Session weighting
...
int OnInit()
{
   ...
   Print("═════════════════════════════════════════");
   Print("  v6 EA — AVAXUSDT 1h | session_wt");
   Print("  R:25% SL:1.0 TP:1.5/5.0/8.0 | minimal");
   Print("  Backtest: 11 trades | 91%WR | PF:6.3");
   Print("  Profit: $+1,673 (+516%) | DD:22%");
   Print("═════════════════════════════════════════");
...
void OpenTrade(int dir)
{
   ...
   G_tr.SetExpertMagicNumber(Inp_BaseMagic);
   if(G_tr.PositionOpen(_Symbol,ot,lots,ep,sl,tp,"AVAXUSDT"))
   {
...
void CheckPending()
{
   ...
   for(int i=ArraySize(G_pend)-1;i>=0;i--)
   {
      ...
      if(fill)
      {
         ...
         G_tr.SetExpertMagicNumber(Inp_BaseMagic);
         if(G_tr.PositionOpen(_Symbol,ot,G_pend[i].lots,ep,sl,tp,"AVAXUSDT PB"))
         {
...
void Dash(int sig)
{
   ...
   d+="══════════════════════════════════════════\n";
   d+="  v6 — AVAXUSDT 1h | session_wt\n";
   d+="  BT: 11T 91%WR PF:6.3 | $+1,673 (+516%)\n";
