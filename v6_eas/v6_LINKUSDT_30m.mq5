//+------------------------------------------------------------------+
//|  v6 EA — LINKUSDT — Optimized for PrimeXBT MT5                    |
//|  Timeframe: 30m | Config: dynamic_tp | Filter: relaxed              |
//|  Backtest (14d): 16 trades | 81% WR | PF 3.5             |
//|  Profit: $+1,972 (+609%) | DD: 49%                      |
//+------------------------------------------------------------------+
#property copyright "v6 EA — LINKUSDT"
#property version   "6.00"
#property strict
#property description "LINKUSDT 30m | dynamic_tp | 16T 81%WR PF:3.5 | $+1,972 (+609%)"

#include <Trade\Trade.mqh>
#include <Trade\SymbolInfo.mqh>

//+------------------------------------------------------------------+
//| Inputs — Pre-optimized for LINKUSDT                                |
//+------------------------------------------------------------------+
input group "════ RISK ════"
...
input int     Inp_BaseMagic    = 638131;
...
input group "════ FILTERS ════"
input double  Inp_VolMin       = 0.4;  // Min vol ratio (ATR14/ATR50)
input double  Inp_VolMax       = 3.0;  // Max vol ratio
input int     Inp_ADXMin       = 20;      // Min ADX
input double  Inp_CandleMinPct = 40;   // Min candle body %
input bool    Inp_UseHTF       = true;  // HTF trend filter
input double  Inp_MinRR        = 1.2;   // Min R:R after spread
...
input bool    Inp_UseDynTP      = true; // Dynamic TP scaling
...
int OnInit()
{
   ...
   Print("═════════════════════════════════════════");
   Print("  v6 EA — LINKUSDT 30m | dynamic_tp");
   Print("  R:25% SL:1.0 TP:1.5/5.0/8.0 | relaxed");
   Print("  Backtest: 16 trades | 81%WR | PF:3.5");
   Print("  Profit: $+1,972 (+609%) | DD:49%");
   Print("═════════════════════════════════════════");
...
int GetSignal()
{
   ...
   double rrEff=(Inp_TP3*a-sp)/(Inp_SL*a*1.5+sp);
...
void OpenTrade(int dir)
{
   ...
   double sl_m=Inp_SL*1.5;  // spread-adjusted SL
   ...
   G_tr.SetExpertMagicNumber(Inp_BaseMagic);
   if(G_tr.PositionOpen(_Symbol,ot,lots,ep,sl,tp,"LINKUSDT"))
   {
...
void CheckPending()
{
   ...
   for(int i=ArraySize(G_pend)-1;i>=0;i--)
   {
      ...
      if(fill)
      {
         ...
         G_tr.SetExpertMagicNumber(Inp_BaseMagic);
         if(G_tr.PositionOpen(_Symbol,ot,G_pend[i].lots,ep,sl,tp,"LINKUSDT PB"))
         {
...
void Dash(int sig)
{
   ...
   d+="══════════════════════════════════════════\n";
   d+="  v6 — LINKUSDT 30m | dynamic_tp\n";
   d+="  BT: 16T 81%WR PF:3.5 | $+1,972 (+609%)\n";
