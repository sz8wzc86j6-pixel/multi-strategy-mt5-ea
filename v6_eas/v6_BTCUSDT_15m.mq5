//+------------------------------------------------------------------+
//|  v6 EA — BTCUSDT — Optimized for PrimeXBT MT5                    |
//|  Timeframe: 15m | Config: pullback30 | Filter: strict              |
//|  Backtest (14d): 8 trades | 100% WR | PF 999.0             |
//|  Profit: $+2,653 (+819%) | DD: 0%                      |
//+------------------------------------------------------------------+
#property copyright "v6 EA — BTCUSDT"
#property version   "6.00"
#property strict
#property description "BTCUSDT 15m | pullback30 | 8T 100%WR PF:999.0 | $+2,653 (+819%)"

#include <Trade\Trade.mqh>
#include <Trade\SymbolInfo.mqh>

//+------------------------------------------------------------------+
//| Inputs — Pre-optimized for BTCUSDT                                |
//+------------------------------------------------------------------+
input group "════ RISK ════"
input double  Inp_Risk         = 15.0;    // Risk % per trade
...
input int     Inp_BaseMagic    = 604632;

input group "════ SL / TP (ATR multiples) ════"
input double  Inp_SL           = 0.3;      // Stop loss × ATR
input double  Inp_TP1          = 0.6;     // Take profit 1 × ATR
input double  Inp_TP2          = 3.0;     // Take profit 2 × ATR
input double  Inp_TP3          = 10.0;     // Take profit 3 × ATR (final)

input group "════ FILTERS ════"
input double  Inp_VolMin       = 0.6;  // Min vol ratio (ATR14/ATR50)
input double  Inp_VolMax       = 2.5;  // Max vol ratio
input int     Inp_ADXMin       = 25;      // Min ADX
input double  Inp_CandleMinPct = 55;   // Min candle body %
input bool    Inp_UseHTF       = true;  // HTF trend filter
input double  Inp_MinRR        = 1.5;   // Min R:R after spread
input double  Inp_MaxSpreadATR = 0.3;
input int     Inp_RSI_OB       = 65;
input int     Inp_RSI_OS       = 35;

input group "════ v6 FEATURES ════"
input bool    Inp_UsePullback   = true;    // Pullback entry
...
int OnInit()
{
   ...
   Print("═════════════════════════════════════════");
   Print("  v6 EA — BTCUSDT 15m | pullback30");
   Print("  R:15% SL:0.3 TP:0.6/3.0/10.0 | strict");
   Print("  Backtest: 8 trades | 100%WR | PF:999.0");
   Print("  Profit: $+2,653 (+819%) | DD:0%");
   Print("═════════════════════════════════════════");
...
void OpenTrade(int dir)
{
   ...
   G_tr.SetExpertMagicNumber(Inp_BaseMagic);
   if(G_tr.PositionOpen(_Symbol,ot,lots,ep,sl,tp,"BTCUSDT"))
   {
...
void CheckPending()
{
   ...
   for(int i=ArraySize(G_pend)-1;i>=0;i--)
   {
      ...
      if(fill)
      {
         ...
         G_tr.SetExpertMagicNumber(Inp_BaseMagic);
         if(G_tr.PositionOpen(_Symbol,ot,G_pend[i].lots,ep,sl,tp,"BTCUSDT PB"))
         {
...
void Dash(int sig)
{
   ...
   d+="══════════════════════════════════════════\n";
   d+="  v6 — BTCUSDT 15m | pullback30\n";
   d+="  BT: 8T 100%WR PF:999.0 | $+2,653 (+819%)\n";
